...
class PGD(BaseAttack):
    ...
    def perturb(self, model, x, label=None, steps=10, step_length=1., min_lambda_=1e-5, max_lambda_=1e5, base=10., verbose=False):
        ...
        if 'k' in list(model.__dict__.keys()) and model.k > 0:
            logger.warning("The attack leads to dense graph and triggers the issue of out of memory.")
        ...
        while self.lambda_ <= max_lambda_:
            ...
            pert_x = self._perturb(model, adv_x[~done], label[~done], steps, step_length, lambda_=self.lambda_)
            adv_x[~done] = pert_x
        ...
        with torch.no_grad():
            ...
            if verbose:
                ...
                logger.info(f"pgd {self.norm}: attack effectiveness {effectiveness:.3f}%.")
