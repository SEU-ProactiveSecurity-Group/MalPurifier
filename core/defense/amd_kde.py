...
class KernelDensityEstimation(DetectorTemplate):
    ...
    def forward(self, x):
        ...
        if len(logits.shape) == 1:
            x_prob = self.forward_g(
                x, logits.argmax().detach())    # 适用于形状为[2]的logits
        else:
            x_prob = self.forward_g(x, logits.argmax(
                1).detach())   # 适用于形状为[10, 2]的logits
