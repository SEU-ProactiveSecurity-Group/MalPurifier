...
class mu_sigma_MLP(nn.Module):
    ...
    def __init__(self, 
        ...
        self.model_save_path = path.join(config.get('experiments', 'mlp') + '_' + self.name, 'model.pth')
    ...
    def train_model(self, vae_model, train_data_producer, batch_size=128, n_epochs=50, verbose=True, device='cuda'):
        ...
        for epoch in range(n_epochs):
            for idx, (inputs, labels) in enumerate(train_data_producer):
                ...
                labels = labels.long()
                one_hot_labels = torch.zeros(labels.size(0), outputs.size(1)).to(device)
                one_hot_labels.scatter_(1, labels.unsqueeze(1), 1)        
                
                loss_classification = criterion_classification(outputs, one_hot_labels)
...
class VAE_2(nn.Module):
    def __init__(self, 
        ...
        # 定义模型的保存路径
        self.model_save_path = path.join(config.get('experiments', 'vae') + '_' + self.name, 'model.pth')
    ...
    def forward(self, x_hat, x, x_comp, label_x, label_x_comp, keep_prob):    
        ...
        # 计算损失函数:
        marginal_likelihood = torch.sum(x * torch.log(y) + (1 - x) * torch.log(1 - y), 1)
        KL_divergence = 0.5 * torch.sum(mu**2 + sigma**2 - torch.log(1e-8 + sigma**2) - 1, 1)
        ...
        loss_1 = torch.mean(torch.abs(F.relu(loss_bac - loss_mean)) * vector_loss)

        # 计算ELBO (证据下界)
        ELBO = self.KLW * torch.mean(marginal_likelihood) - torch.mean(KL_divergence)
        loss = -ELBO
    ...
    def train_model(self, train_data_producer, batch_size=128, n_epochs=10, verbose=True, device='cuda'):
        ...
        for epoch in range(n_epochs):
            ...
            for i in range(total_batch):
                ...
                # 获取良性和恶意的批次数据
                batch_ben_input_s = ben_data_combined[offset:(offset + batch_size), :]
                batch_mal_input_s = mal_data_combined[offset:(offset + batch_size), :]
                batch_ben_input_s_cpu = batch_ben_input_s.cpu().numpy()
                batch_mal_input_s_cpu = batch_mal_input_s.cpu().numpy()
                batch_input = np.row_stack((batch_ben_input_s_cpu, batch_mal_input_s_cpu))
                ...
                # 获取目标数据
                batch_xs_target = ben_data_combined[offset:(offset + batch_size), :]
                batch_xs_target = torch.cat((batch_xs_target, batch_xs_target), dim=0)
                ...
                if isinstance(batch_xs_input, np.ndarray):
                    batch_xs_input = torch.tensor(batch_xs_input, dtype=torch.float32).to(device)

                if isinstance(batch_xs_target, np.ndarray):
                    batch_xs_target = torch.tensor(batch_xs_target, dtype=torch.float32).to(device)

                if isinstance(batch_xs_label, np.ndarray):
                    batch_xs_label = torch.tensor(batch_xs_label, dtype=torch.float32).to(device)

                if isinstance(batch_xcomp_label, np.ndarray):
                    batch_xcomp_label = torch.tensor(batch_xcomp_label, dtype=torch.float32).to(device)

                if isinstance(batch_xcomp_input, np.ndarray):
                    batch_xcomp_input = torch.tensor(batch_xcomp_input, dtype=torch.float32).to(device)
            ...
            # 打印每个周期的损失信息
            logger.info(f"epoch[{epoch}/{n_epochs}]: L_tot {loss.item():.2f} L_likelihood {marginal_likelihood.item():.2f} L_divergence {KL_divergence.item():.2f}")
...
class VAE_SU(nn.Module, DetectorTemplate):
    def __init__(self, 
        ...
        # 日志中打印模型的结构信息
        logger.info('=====================================fd_vae model architecture=============================')
        logger.info(self)
        logger.info('===============================================end==========================================')
    ...
    def fit(self, train_data_producer, verbose=True):
        ...
        # train Mlp
        self.Mlp.train_model(vae_model, train_data_producer, device=self.device)
    ...
    def inference(self, test_data_producer):
        ...
        with torch.no_grad():
            for x, l in test_data_producer:
                ...
                gt_labels.append(l)  # Store the actual labels instead of the VAE output  
    ...
    def inference_batch_wise(self, x):
        ...
        x_cent = x_cent_values[:2] if len(x_cent_values.shape) == 1 else x_cent_values[:, :2]
