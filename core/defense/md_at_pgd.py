"""
Adversarial training incorporating PGD L-inf attack
"""
...
class PGDAdvTraining(object):
    ...
    Parameters
    -------
    model: Object, model to be protected, e.g. MalwareDetector
        ...
        for i in range(adv_epochs):
            ...
            for idx_batch, (x_batch, y_batch) in enumerate(train_data_producer):
                ...
                if verbose:
                    ...
                    logger.info(
                        f'Training loss: {losses[-1]:.4f} | Training accuracy: {acc_train * 100:.2f}%.')
            if verbose:
                logger.info(
            ...
            if verbose:
                logger.info(
                    f"\tValidation accuracy {acc_val * 100:.4}% with accuracy {acc_val_adv * 100:.4}% under attack.")
                logger.info(
                    f"\tModel selected at epoch {best_epoch} with validation accuracy {best_acc_val * 100:.4}% and accuracy {acc_val_adv_be * 100:.4}% under attack.")
