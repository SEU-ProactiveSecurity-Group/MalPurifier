...
class MalwareDetectionRNN(nn.Module):
    ...
    def forward(self, x):
        ...
        if len(x.shape) == 1:  # 如果是1D张量
            x = x.unsqueeze(0).unsqueeze(1)  # 这将[sequence_length]变为[1, 1, sequence_length]
        elif len(x.shape) == 2:  # 如果是2D张量
            x = x.unsqueeze(1)  # 这将[sequence_length, num_features]变为[1, sequence_length, num_features]
