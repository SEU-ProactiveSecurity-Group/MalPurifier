...
import numpy as np
import torch
from core.defense import Dataset, MalwareDetectionDNN, PGDAdvTraining, RFGSMAdvTraining, MaxAdvTraining, KernelDensityEstimation, \
...
def _main():
    ...
    save_dir = config.get('experiments', args.model) + '_' + args.model_name
    ...
    # Load the appropriate model based on args.model
    if args.model in ['md_at_pgd', 'md_at_ma', 'md_at_fgsm']:
        at_wrapper_class = {'md_at_pgd': PGDAdvTraining, 'md_at_ma': MaxAdvTraining, 'md_at_fgsm': RFGSMAdvTraining}[args.model]
        at_wrapper = at_wrapper_class(model)
        at_wrapper.load()
        model = at_wrapper.model
    elif args.model == 'amd_kde':
        model = KernelDensityEstimation(model, n_centers=hp_params['n_centers'], bandwidth=hp_params['bandwidth'],
                                        n_classes=dataset.n_classes, ratio=hp_params['ratio'])
        model.load()
    elif args.model in ['amd_icnn', 'amd_pad_ma']:
        model = AdvMalwareDetectorICNN(model, input_size=dataset.vocab_size, n_classes=dataset.n_classes,
                                       device=dv, name=args.model_name, **hp_params)
        if args.model == 'amd_pad_ma':
            adv_model = AMalwareDetectionPAD(model)
            adv_model.load()
            model = adv_model.model
        else:
            model.load()
    elif args.model in ['amd_dla', 'amd_dnn_plus']:
        model_class = AMalwareDetectionDLA if args.model == 'amd_dla' else AMalwareDetectionDNNPlus
        model = model_class(md_nn_model=None, input_size=dataset.vocab_size, n_classes=dataset.n_classes,
                            device=dv, name=args.model_name, **hp_params)
        ...
        model = DAE(input_size=dataset.vocab_size, device=dv, name=args.model_name, predict_model_name=args.basic_dnn_name)
        model.load()
        model.to(dv)
        
        predict_model = MalwareDetectionDNN(dataset.vocab_size, dataset.n_classes, device=dv, 
                                            name=model.predict_model_name, **hp_params)
        predict_model = predict_model.to(dv).double()
        predict_model_save_path = os.path.join(config.get('experiments', 'md_dnn') + '_' + model.predict_model_name, 'model.pth')
    ...
    logger.info(f"Load model parameters from {model.model_save_path}.")
    model.eval()
    ...
    if args.model == 'dae':
        for x, y in mal_test_dataset_producer:
            ...
            _flag, x_mod = attack.perturb_dae(model, predict_model, x, trials=args.trials, seed=0, is_apk=args.real, verbose=True)
            success_flag_list.append(_flag)
            logger.info(f"The attack effectiveness under mimicry attack is {np.sum(_flag) / float(len(_flag)) * 100}%.")
            x_mod_list.append(x_mod)
    else:
        for x, y in mal_test_dataset_producer:
            x, y = utils.to_tensor(x.double(), y.long(), model.device)
            _flag, x_mod = attack.perturb(model, x, trials=args.trials, seed=0, is_apk=args.real, verbose=True)
            success_flag_list.append(_flag)
            logger.info(f"The attack effectiveness under mimicry attack is {np.sum(_flag) / float(len(_flag)) * 100}%.")
            x_mod_list.append(x_mod)
    ...
    if args.real:
        ...
        utils.dump_pickle_frd_space(x_mod_list, os.path.join(save_dir, 'x_mod.list'))
